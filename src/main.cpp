...
{   
    ...
    // Kurangi 1 tahun jika bulan saat ini kurang dari bulan lahir, 
    // atau jika bulannya sama tetapi tanggal saat ini kurang dari tanggal lahir
...
int monthsOld(tm* inputTgl, tm* currentTgl)
{
    ...
    int months = (currentTgl->tm_year - inputTgl->tm_year) * 12 + (currentTgl->tm_mon - inputTgl->tm_mon);
    
    // Kurangi 1 bulan jika tanggal saat ini masih sebelum tanggal lahir di bulan tersebut
...
string dayOfDate(tm* inputTgl)
{   
    // Set tm_isdst (Daylight Saving Time) ke -1 agar mktime menentukannya sendiri
    inputTgl->tm_isdst = -1; 
    
    // Memanggil mktime untuk menormalisasi struct tm dan mengisi tm_wday
    mktime(inputTgl);
    
    // Array nama hari, dimulai dari indeks 0 (Minggu) sesuai dengan standar tm_wday
